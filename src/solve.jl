...
scalecoef(rows, coef, minus, s::Union{MOI.LessThan, MOI.Nonpositives}) = minus ? coef : -coef
...
function constrcall(I, J, V, b, varmap::Dict, constrmap::Dict, cr, f::MOI.VectorAffineFunction, s::LPCones)
    ...
    # The SCS format is b - Ax ∈ cone
    # so minus=false for b and minus=true for A
    b[i] = scalecoef(rows, f.constant, false, s)
...
function MOI.optimize!(instance::SCSSolverInstance)
    ...
    display(full(A))
    sol = SCS_solve(m, n, A, b, c, cone.f, cone.l, cone.qa, length(cone.q), cone.sa, length(cone.s), cone.ep, cone.ed, cone.p, length(cone.p))
    instance.ret_val = sol.ret_val
    instance.primal = sol.x
    instance.dual = sol.y
    instance.slack = sol.s
    instance.objval = dot(c0, instance.primal)
end
